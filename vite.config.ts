...
export default defineConfig(({mode}) => {
  ...
  const config = {
    ...
    appId: env.VITE_FIREBASE_APP_ID || process.env.FIREBASE_APP_ID,
    apiKey: env.VITE_FIREBASE_API_KEY || process.env.FIREBASE_API_KEY,
    authDomain: env.VITE_FIREBASE_AUTH_DOMAIN || process.env.FIREBASE_AUTH_DOMAIN,
    storageBucket: env.VITE_FIREBASE_STORAGE_BUCKET || process.env.FIREBASE_STORAGE_BUCKET,
  ...
  for (const key of required) {
    // @ts-ignore
    if (!config[key]) {
      throw new Error(`Missing required Firebase config: ${key}. Set VITE_FIREBASE_${key.toUpperCase()} or FIREBASE_${key.toUpperCase()}.`);
    }
  }
