...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    // ⚡ Bolt Optimization: Hoist constant date and string parsing outside the O(N) filter loop
    const filterStartMs = filterStartDate ? new Date(filterStartDate).getTime() : 0;
    const filterEndMs = filterEndDate ? new Date(filterEndDate).getTime() + 86400000 : 0; // Include full end day
    ...
    let result = invoices.filter(inv => {
      ...
      if (filterStartMs && inv.uploadedAt && inv.uploadedAt < filterStartMs) return false;
      if (filterEndMs && inv.uploadedAt && inv.uploadedAt > filterEndMs) return false;
  ...
  const { approvedPercent, topVendors, dailyData, totalApproved, totalFlagged, avgConfidence } = useMemo(() => {
    let approved = 0;
    let flagged = 0;
    let totalScore = 0;
    let scoreCount = 0;
    ...
    invoices.forEach(inv => {
      // ⚡ Bolt Optimization: Calculate totals here instead of inline filtering in JSX
      ...
      // Calculate overall average confidence directly here
    ...
    return { approvedPercent, topVendors, dailyData, totalApproved: approved, totalFlagged: flagged, avgConfidence };
  }, [invoices]);
