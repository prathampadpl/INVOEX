...
export default function Export() {
  ...
  const filteredInvoices = React.useMemo(() => {
    ...
    // Wrap derived list in useMemo to prevent redundant filtering on every render.
    // Hoist expensive string/date operations outside the loop.
    const vendorSearch = filterVendor ? filterVendor.toLowerCase() : '';
    const startMs = filterStartDate ? new Date(filterStartDate).getTime() : null;
    const endMs = filterEndDate ? new Date(filterEndDate).getTime() : null;

    return invoices.filter(i => {
       if (filterStatus !== 'All' && i.status !== filterStatus) return false;
       if (vendorSearch && !i.vendorName?.toLowerCase().includes(vendorSearch)) return false;

       if (i.invoiceDate) {
          const invMs = new Date(i.invoiceDate).getTime();
          if (startMs && invMs < startMs) return false;
          if (endMs && invMs > endMs) return false;
       }
