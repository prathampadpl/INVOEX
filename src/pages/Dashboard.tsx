...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    let result = invoices.filter(inv => {
      ...
      if (startMs && inv.uploadedAt && inv.uploadedAt < startMs) return false;
      if (endMs && inv.uploadedAt && inv.uploadedAt > endMs) return false;
  ...
  const { approvedPercent, topVendors, dailyData, approvedCount, flaggedCount, globalAvgConf } = useMemo(() => {
    let approved = 0;
    let flagged = 0;
    let totalConfSum = 0;
    let confCount = 0;
    ...
    invoices.forEach(inv => {
      ...
      let avgConf = 0;
      if (scores.length) {
         const scoreSum = scores.reduce((a: number, b: number) => a + b, 0);
         avgConf = scoreSum / scores.length;
         totalConfSum += scoreSum;
         confCount += scores.length;
      }
    ...
    const globalAvgConf = confCount ? (totalConfSum / confCount).toFixed(1) : '0.0';
    ...
    return { approvedPercent, topVendors, dailyData, approvedCount: approved, flaggedCount: flagged, globalAvgConf };
  }, [invoices]);
