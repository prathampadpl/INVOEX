...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    // Hoist expensive operations outside the filter loop
  ...
  const { approvedPercent, topVendors, dailyData, overallAvgConfidence } = useMemo(() => {
    ...
    // Pre-instantiate DateTimeFormat to avoid repeated O(N) instantiations
      ...
      if (inv.confidenceScores) {
        for (const field in inv.confidenceScores) {
          if (inv[field] !== undefined && inv[field] !== null && inv[field] !== '') {
             invConfSum += inv.confidenceScores[field];
             invConfCount++;
          }
        }
      }

      const avgConf = invConfCount > 0 ? invConfSum / invConfCount : 0;

      if (invConfCount > 0) {
        globalConfSum += invConfSum;
        globalConfCount += invConfCount;
      }
    ...
    const overallAvgConfidence = globalConfCount > 0 ? (globalConfSum / globalConfCount).toFixed(1) : '0.0';
                  ...
                  <TableRow key={inv.id} className={inv.status === 'Failed' ? 'bg-red-50 hover:bg-red-100' : 'border-gray-100 hover:bg-gray-50'}>
                    ...
                    <TableCell>
                      {inv.confidenceScores ? (() => {
                        ...
                        for (const field in inv.confidenceScores) {
                          if (inv[field] !== undefined && inv[field] !== null && inv[field] !== '') {
                            sum += inv.confidenceScores[field] as number;
                            count++;
                          }
                        }
