...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    const filterEndMs = filterEndDate ? new Date(filterEndDate).getTime() + 86400000 : 0;

    let result = invoices.filter(inv => {
      if (statusFilter !== 'All statuses' && inv.status !== statusFilter) return false;

      if (searchQueryLower) {
        if (!inv.vendorName?.toLowerCase().includes(searchQueryLower) && !inv.invoiceNumber?.toLowerCase().includes(searchQueryLower)) {
          return false;
        }
      }
  ...
  const { approvedPercent, topVendors, dailyData, autoApprovedCount, flaggedCount, globalAvgConf } = useMemo(() => {
    let approved = 0;
    ...
    invoices.forEach(inv => {
      ...
      if (inv.confidenceScores) {
         ...
         invAvgConf = invScoreCount > 0 ? invScoreSum / invScoreCount : 0;
      }
    ...
    const globalAvgConf = totalScoreCount > 0 ? (totalScoreSum / totalScoreCount).toFixed(1) : '0.0';
    ...
    return { approvedPercent, topVendors, dailyData, autoApprovedCount, flaggedCount, globalAvgConf };
  }, [invoices]);
