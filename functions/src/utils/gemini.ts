  ...
  for (const model of modelChain) {
    try {
      ...
      if (model.type === 'gemini') {
        return await tryModelWithTimeout((signal) => tryGeminiModel(model.name, buffer, mimeType, correctionsLogString, knownVendorsString, signal));
      } else {
        return await tryModelWithTimeout((signal) => tryOpenRouterModel(model.name, buffer, mimeType, correctionsLogString, knownVendorsString, signal));
      }
    } catch (err: any) {
