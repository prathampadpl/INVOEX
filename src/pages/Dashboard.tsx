...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    let result = invoices.filter(inv => {
      ...
      if (filterEndDate) { // Check filterEndDate to avoid using Infinity check if it wasn't set
  ...
  const { approvedPercent, topVendors, dailyData, avgConfidence } = useMemo(() => {
    ...
    invoices.forEach(inv => {
      ...
      // Calculate individual confidence sum and count to also use for overall avg
      if (inv.confidenceScores) {
        ...
        if (scores.length) {
            ...
            if (inv.vendorName) {
                ...
                vendors[inv.vendorName].confSum += (sum / scores.length);
            }
        } else if (inv.vendorName) {
            // Handle case where scores length is 0 but vendor name exists
            if (!vendors[inv.vendorName]) vendors[inv.vendorName] = { count: 0, confSum: 0 };
            vendors[inv.vendorName].count++;
        }
      } else if (inv.vendorName) {
          // Handle case where no confidence scores but vendor name exists
          if (!vendors[inv.vendorName]) vendors[inv.vendorName] = { count: 0, confSum: 0 };
          vendors[inv.vendorName].count++;
      }
    ...
    const topVendors = Object.entries(vendors)
      .map(([name, stats]) => ({ name, count: stats.count, avgConf: stats.count > 0 ? stats.confSum / stats.count : 0 }))
      .sort((a, b) => b.count - a.count)
      .slice(0, 5);

    const dailyData = Object.values(days).slice(-14);

    const avgConfidence = totalScoreCount > 0 ? (totalScoreSum / totalScoreCount).toFixed(1) : "0";
        ...
        <Card className="shadow-sm border-gray-200">
          ...
          <CardContent className="flex items-end justify-between">
            <div className="text-3xl font-bold text-gray-900">
             <>{avgConfidence === "0" ? "0.0" : avgConfidence}%</>
            </div>
