...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    // Hoist constants out of the O(N) filter loop
    ...
    let result = invoices.filter(inv => {
      ...
      if (startTime) {
        if (inv.uploadedAt && inv.uploadedAt < startTime) return false;
      }
      if (endTime) {
        if (inv.uploadedAt && inv.uploadedAt > endTime) return false;
      }
  ...
  const { approvedPercent, topVendors, dailyData, approvedCount, flaggedCount, avgConfidence } = useMemo(() => {
    let approvedCount = 0;
    let flaggedCount = 0;
    ...
    invoices.forEach(inv => {
      ...
      if (inv.confidenceScores) {
        ...
        if (scores.length) {
          const avgConf = scores.reduce((a: number, b: number) => a + b, 0) / scores.length;
          if (inv.vendorName) vendors[inv.vendorName].confSum += avgConf;

          totalConfidenceSum += scores.reduce((a: number, b: number) => a + b, 0);
          confidenceItemCount += scores.length;
        }
      }
    ...
    const approvedPercent = invoices.length ? Math.round((approvedCount / invoices.length) * 100) : 0;
    const avgConfidence = confidenceItemCount ? (totalConfidenceSum / confidenceItemCount).toFixed(1) : '0.0';
    ...
    return { approvedPercent, topVendors, dailyData, approvedCount, flaggedCount, avgConfidence };
  }, [invoices]);
