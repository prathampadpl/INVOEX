...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    // ⚡ Bolt: Hoist expensive query normalizations out of the array.filter loop
  ...
  const { approvedPercent, topVendors, dailyData } = useMemo(() => {
    ...
    // ⚡ Bolt: Cache Intl formatter to avoid instantiating it per invoice
    ...
    for (let i = 0; i < invoices.length; i++) {
      ...
      if (inv.uploadedAt) {
        const d = dateFormatter.format(inv.uploadedAt);
