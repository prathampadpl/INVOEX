...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    // Hoisted static date parsing and string lowercasing outside the O(N) filter loop
    // to prevent redundant recalculation on every item.
  ...
  // ⚡ Bolt Performance Optimization:
  // Consolidated O(N) inline JSX aggregations (like totalApproved and avg confidence)
  // into this single useMemo pass to avoid expensive recalculations on pure UI re-renders (e.g. typing in search).
  const { approvedPercent, topVendors, dailyData, totalApproved, totalFlagged, overallAvgConfidence } = useMemo(() => {
    let approved = 0;
    let flagged = 0;
    ...
    // ⚡ Bolt Performance Optimization:
    // Cached Intl.DateTimeFormat instance outside the O(N) loop to avoid the massive
    // overhead of instantiating formatters inside .toLocaleDateString() on every iteration.
    ...
    invoices.forEach(inv => {
      ...
      if (inv.confidenceScores) {
         ...
         if (scores.length) {
            ...
            currentInvAvgConf = sum / scores.length;
            allScoresSum += sum;
      ...
      if (inv.vendorName) {
        ...
        vendors[inv.vendorName].confSum += currentInvAvgConf;
      }
    ...
    const overallAvgConfidence = allScoresCount ? (allScoresSum / allScoresCount).toFixed(1) : '0.0';

    return { approvedPercent, topVendors, dailyData, totalApproved: approved, totalFlagged: flagged, overallAvgConfidence };
  }, [invoices]);
    ...
    <div className="w-full max-w-[1200px] mx-auto space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="shadow-sm border-gray-200">
          ...
          <CardContent className="flex items-end justify-between">
            <div className="text-3xl font-bold text-gray-900">{overallAvgConfidence}%</div>
