...
function isRetryableError(err: any): boolean {
  ...
  // 429 Too Many Requests, 503 Service Unavailable, 504 Gateway Timeout, 502 Bad Gateway
...
async function tryGeminiModel(modelName: string, buffer: Buffer, mimeType: string, signal: AbortSignal): Promise<{ data: any[], extractedBy: string }> {
...
async function tryOpenRouterModel(modelName: string, buffer: Buffer, mimeType: string, signal: AbortSignal): Promise<{ data: any[], extractedBy: string }> {
  ...
  for (const model of modelChain) {
    try {
      ...
      if (model.type === 'gemini') {
        return await tryModelWithTimeout((signal) => tryGeminiModel(model.name, buffer, mimeType, signal));
      } else {
        return await tryModelWithTimeout((signal) => tryOpenRouterModel(model.name, buffer, mimeType, signal));
      }
    } catch (err: any) {
