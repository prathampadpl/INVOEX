...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    // Hoist expensive parsing outside the filter loop
    ...
    let result = invoices.filter(inv => {
      ...
      if (startMs && inv.uploadedAt) {
        if (inv.uploadedAt < startMs) return false;
      }
      if (endMs && inv.uploadedAt) {
        if (inv.uploadedAt > endMs) return false;
      }
  ...
  const { approvedPercent, overallAvgConfidence, topVendors, dailyData } = useMemo(() => {
    let approved = 0;
    ...
    // Hoist formatter and overall confidence trackers
    ...
    invoices.forEach(inv => {
      ...
      if (inv.confidenceScores) {
        ...
        avgConf = scoreCount > 0 ? scoreSum / scoreCount : 0;
      }
    ...
    const overallAvgConfidence = totalScoreCount > 0 ? (totalScoreSum / totalScoreCount).toFixed(1) : '0.0';
    ...
    return { approvedPercent, overallAvgConfidence, topVendors, dailyData };
  }, [invoices]);
